...
def tcomplex(x):
    if isinstance(x, tany):
        return x.create(complex(x.value))
    else:
        return complex(x)
    
for op in ops + rops:
    setattr(tany, op, tany.make_wrapper(op))
...
for op in container_ops_unchanged:
    setattr(tany, op, tany.make_wrapper_unchanged_return(op))
